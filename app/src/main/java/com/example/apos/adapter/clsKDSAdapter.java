...
public  class clsKDSAdapter extends BaseAdapter
{
    ...
    public clsKDSAdapter(Context context, Map<String,  List<clsBillDtl>> hmKOTDtl,List<clsBillDtl> arrListItem, Map<String,  String> hmSelectedKOTDtl) {
        ...
        else
        {
          ...
          if(hmKOTDtl.size()>0)
          {
              if(hmKOTDtl.containsKey(objKOT.getStrKOTNo()))
              {
                  ...
                  clsKDSListItemAdapter adapter= new clsKDSListItemAdapter(clsKDSScreenFotKOT.mActivity,arrKOTListItem);
                  adapter.setCustomButtonListner((clsKDSListItemAdapter.customButtonListener) clsKDSScreenFotKOT.mActivity);
