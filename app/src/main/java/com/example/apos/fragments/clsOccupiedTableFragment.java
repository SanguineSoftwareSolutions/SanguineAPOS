...
public class clsOccupiedTableFragment extends Fragment
{

    private GridView occupiedTableGridview;
    public ArrayList<clsTableMaster> arrListAllTableMaster;
    private ConnectivityManager connectivityManager;
    private String posCode="",areaCode="";
    private int paxCount=0;
    public static clsOccupiedTableFragment getInstance()
    {
        clsOccupiedTableFragment tabOccupiedTable = new clsOccupiedTableFragment();
        return tabOccupiedTable;
    }
    ...
    {
        View rootView = inflater.inflate(R.layout.viewoccupiedtabletab, container, false);
        connectivityManager = (ConnectivityManager) getActivity().getSystemService(Context.CONNECTIVITY_SERVICE);
        posCode= getArguments().getString("POSCode");
        areaCode= getArguments().getString("AreaCode");
        occupiedTableGridview = (GridView) rootView.findViewById(R.id.viewOccupiedTable_Gridview);
        funGetOccupiedTable();
        return rootView;
    }

    public void funGetOccupiedTable()
    {
        if (!StringUtils.isEmpty(clsGlobalFunctions.gAPOSWebSrviceURL)) {
            if (ConnectivityHelper.isConnected())
            {
                App.getAPIHelper().funGetTableStatus(posCode,"Occupied",areaCode, new BaseAPIHelper.OnRequestComplete<ArrayList<clsTableMaster>>() {
                    @Override
    ...
    private void funFillTableGrid(ArrayList arrListTableMaster)
    {
        ...
        occupiedTableGridview.setAdapter(new clsViewTableStatusAdapter(getActivity(), this.getActivity(), arrListTableMaster,"Occupied"));
    }
