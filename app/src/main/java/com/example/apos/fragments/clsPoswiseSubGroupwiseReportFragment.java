...
public class clsPoswiseSubGroupwiseReportFragment extends Fragment
{
    private GridView gSubGroupWiseGridView;
    clsPoswiseSalesMenuAdapter objAdapter;
    List<clsSalesReportDtl> arrListSubGroupWise = new ArrayList();
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container,
        ...
        gSubGroupWiseGridView= (GridView) (myFragmentView).findViewById(R.id.gvPoswiseSalesList);
        funGetSubGroupWiseReportDtl();
        return myFragmentView;
    }


    public void funGetSubGroupWiseReportDtl()
    {
        if (!StringUtils.isEmpty(clsGlobalFunctions.gAPOSWebSrviceURL)) {
            if (ConnectivityHelper.isConnected())
            {
                showDialog();
                App.getAPIHelper().funPOSSalesReport(fromDate,toDate,"SubGroupWise", new BaseAPIHelper.OnRequestComplete<ArrayList<clsSalesReportDtl>>() {
                    @Override
                    public void onSuccess(ArrayList<clsSalesReportDtl> arrListTemp)
                    {
                        ...
                        if (null != arrListTemp)
                        {
                            if (arrListTemp.size()>0)
                            {
                                ...
                                arrListSubGroupWise=arrListTemp;
                                if(arrListSubGroupWise.size()>0)
                                {
                                    arrPosNameList.add("Total");
                                    objAdapter=new clsPoswiseSalesMenuAdapter(getActivity(),arrListSubGroupWise,arrPosNameList,"SubGroupWise");
                                    gSubGroupWiseGridView.setAdapter(objAdapter);
                                }
                            }
                        }
                    }
