...
public class clsPoswiseMenuheadwiseReportFragment extends Fragment
{
    private GridView gMenuWiseGridView;
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container,
        ...
        gMenuWiseGridView= (GridView) (myFragmentView).findViewById(R.id.gvPoswiseSalesList);
        funGetMenuHeadWiseReportDtl();
        return myFragmentView;
    }


    public void funGetMenuHeadWiseReportDtl()
    {
        if (!StringUtils.isEmpty(clsGlobalFunctions.gAPOSWebSrviceURL)) {
            if (ConnectivityHelper.isConnected())
            {
                showDialog();
                App.getAPIHelper().funPOSSalesReport(fromDate,toDate,"MenuHeadWise", new BaseAPIHelper.OnRequestComplete<ArrayList<clsSalesReportDtl>>() {
                    @Override
                    public void onSuccess(ArrayList<clsSalesReportDtl> arrListTemp)
                    {
                        ...
                        if (null != arrListTemp)
                        {
                            if (arrListTemp.size()>0)
                            {
                                ...
                                if(arrListMenuWise.size()>0)
                                {
                                    arrPosNameList.add("Total");
                                    objAdapter=new clsPoswiseSalesMenuAdapter(getActivity(),arrListMenuWise,arrPosNameList,"MenuWise");
                                    gMenuWiseGridView.setAdapter(objAdapter);
                                }
                            }
                        }
                    }
