...
public class clsPoswiseGroupwiseReportFragment extends Fragment
{
    private GridView gGroupWiseGridView;
    clsPoswiseSalesMenuAdapter objAdapter;
    List<clsSalesReportDtl> arrListGroupWise = new ArrayList();
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container,
        ...
        gGroupWiseGridView= (GridView) (myFragmentView).findViewById(R.id.gvPoswiseSalesList);

        funGetGroupWiseReportDtl();
        return myFragmentView;
    }


    public void funGetGroupWiseReportDtl()
    {
        if (!StringUtils.isEmpty(clsGlobalFunctions.gAPOSWebSrviceURL)) {
            if (ConnectivityHelper.isConnected())
            {
                showDialog();
                App.getAPIHelper().funPOSSalesReport(fromDate,toDate,"GroupWise", new BaseAPIHelper.OnRequestComplete<ArrayList<clsSalesReportDtl>>() {
                    @Override
                    public void onSuccess(ArrayList<clsSalesReportDtl> arrListTemp)
                    {
                        ...
                        if (null != arrListTemp)
                        {
                            if (arrListTemp.size()>0)
                            {
                                ...
                                arrListGroupWise=arrListTemp;
                                if(arrListGroupWise.size()>0)
                                {
                                    arrPosNameList.add("Total");
                                    objAdapter=new clsPoswiseSalesMenuAdapter(getActivity(),arrListGroupWise,arrPosNameList,"GroupWise");
                                    gGroupWiseGridView.setAdapter(objAdapter);
                                }
